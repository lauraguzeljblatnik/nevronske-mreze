...
\povzetek{Diplomsko delo se poglobi v usmerjene nevronske mreže. Le te temeljijo na posnemanju možganskih funkcij, uporabljajo pa se za napovedovanje in klasifikacijo. Sestavljajo jih nevroni organizirani v sloje in med sabo povezani s sinapsami. S pomočjo algoritma vzvratnega razširjanja napake in učnih primerov mrežo naučimo, da se zna odzvati na neznane situacije. Algoritem temelji na spreminjanju uteži na sinapsah, učenje poteka dokler ni napaka med želeno in izračunano vrednostjo dovolj majhna. Poleg nevronskih mrež se delo osredotoči tudi na funkcijsko programiranje s poudarkom na programskem jeziku OCaml. Na koncu je primer nevronske mreže tudi implementiran, na konkretnem primeru je analizirano delovanje mreže.}

%  Prevod slovenskega povzetka v angleščino.
\abstract{The paper focuses on feed-forward neural networks. Mimicking brain functions, neural networks are used for prediction and classification. Neural networks are composed of neurons organized in layers and connected by synapses. With the backpropagation algorithm and learning examples the network is able to learn how to respond to unknown situations. Idea behind the backpropagation is to change weights on the synapses until error between computed and desired values in small enough. In addition, paper presents ideas behind functional programming and programming language OCaml. Implementation of neural network and it's analysis is presented in the end of the paper.}
...
Nabor podatkov vsebuje informacije o izposoji koles za $731$ dni, od tega sem $600$ primerov uporabila za učenje, na $131$ primerih pa sem testirala delovanje nevronske mreže.
%
...
Da je bilo učenje nevronske mreže uspešno, pokažemo s funkcijo\-  \texttt{learned\-\_vs\-\_unlearned}. Funkcija za n primerov primerja povprečno absolutno napako za naučeno in nenaučeno mrežo. Če je napaka pri naučeni mreži manjša, v tabelo na mesto ustreznega izhodnega parametra prištejemo vrednost $1$, če je napaka manjša pri nenaučeni mreži vrednost $1$ odštejemo.
...
\begin{figure}[!h]
	...
	\label{fig:rate}
\end{figure}
Tako me je zanimalo, kako stopnja učenja vpliva na delovanje nevronske mreže. Pri fiksni topologiji z eno skrito plastjo in desetimi nevroni v njej sem spreminjala stopnjo učenja. Iz grafa~\ref{fig:rate}, ki prikazuje povprečno absolutno napako pri petdesetih ponovitvah učenja v odvisnosti od stopnje učenja, je razvidno, da je napaka najmanjša, pri vrednostih $\eta = 0.5$ in $\eta = 1$, tako bo mreža delovala najbolje, če za $\eta$ izberemo število med tema dvema vrednostima.
